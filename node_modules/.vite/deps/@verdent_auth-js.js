...
// node_modules/@verdent/auth-js/dist/i18n.js
...
function normalizeLocale(value) {
  ...
  if (primary === "zh") {
    return rest.includes("hant") || rest.includes("tw") || rest.includes("hk") || rest.includes("mo") ? "zh-Hant" : "zh-CN";
  }
...
function resolveAuthLocale(windowRef, locale) {
  var _a;
  if (locale !== "auto")
  ...
  const browserLocales = ((_a = windowRef.navigator.languages) == null ? void 0 : _a.length) ? windowRef.navigator.languages : [windowRef.navigator.language];
...
function resolveAuthMessages(locale, overrides) {
...
// node_modules/@verdent/auth-js/dist/index.js
var DEFAULT_AUTHORIZE_PATH = "/auth/v1/authorize";
var DEFAULT_OAUTH_TIMEOUT_MS = 5 * 60 * 1e3;
var DEFAULT_EMAIL_PLACEHOLDER = "verdent@gmail.com";
var VERDENT_CLOUD_OAUTH_ORIGIN = "https://cloud-oauth.verdent.ai";
var VERDENT_PROD_SUPABASE_API_ORIGIN = "https://supabase-api-prod.verdent.ai";
var VerdentAuthError = class extends Error {
...
function createVerdentAuth(options) {
  var _a;
  if (!((_a = options == null ? void 0 : options.supabase) == null ? void 0 : _a.auth)) {
...
function resolveOAuthTarget(options, windowRef) {
  ...
  if (((_a = options.oauth) == null ? void 0 : _a.authorizeUrl) !== void 0) {
    const projectRef2 = projectRefFromSupabaseUrl(supabaseUrl);
    const authorizeUrl = normalizeURL("oauth.authorizeUrl", options.oauth.authorizeUrl);
    const parsed = new URL(authorizeUrl);
    if (isManagedOAuthInitiatePath(parsed.pathname) && !projectRef2) {
    ...
    return {
      ...
      ...projectRef2 ? { projectRef: projectRef2 } : {}
    };
  }
  if (!supabaseUrl) {
...
function waitForOAuthResult(options, windowRef, popup, trustedResponseOrigin, state) {
  return new Promise((resolve, reject) => {
    ...
    }, ((_a = options.oauth) == null ? void 0 : _a.timeoutMs) ?? DEFAULT_OAUTH_TIMEOUT_MS);
...
function isTrustedOAuthResponseOrigin(origin, trustedResponseOrigin) {
  ...
  try {
    ...
    return parsed.protocol === "https:" && parsed.port === "" && /^[a-z0-9-]*oauth\.verdent\.ai$/.test(parsed.hostname);
  } catch {
    return false;
  }
}
...
function isVerdentProductionSupabaseURL(url) {
  return url.origin === VERDENT_PROD_SUPABASE_API_ORIGIN || url.hostname.endsWith(".preview.verdent.ai") || url.hostname.endsWith(".verdent.app");
}
...
function readSupabaseClientUrl(supabase) {
  ...
  return typeof clientSupabaseUrl === "string" && clientSupabaseUrl.trim() ? clientSupabaseUrl.trim() : void 0;
}
function projectRefFromSupabaseUrl(rawUrl) {
  const value = rawUrl == null ? void 0 : rawUrl.trim();
  if (!value)
    return void 0;
  try {
...
function resolveTheme(windowRef, theme) {
  ...
  if (theme === "light" || theme === "dark")
    return theme;
  if ((_a = windowRef.matchMedia) == null ? void 0 : _a.call(windowRef, "(prefers-color-scheme: dark)").matches)
    return "dark";
  return "light";
}
...
function renderAuthFlow(options, windowRef, container, uiOptions, onAuthenticated) {
  ...
  const replaceCard = (card) => {
    currentCard == null ? void 0 : currentCard.remove();
    currentCard = card;
    container.append(card);
  };
  const complete = async (result) => {
    await notifyAuthSuccess(result, uiOptions);
    await (onAuthenticated == null ? void 0 : onAuthenticated());
  };
  const renderSignIn = () => {
...
async function notifyAuthSuccess(result, options) {
  var _a;
  try {
    await ((_a = options.onSuccess) == null ? void 0 : _a.call(options, result, { action: result.action }));
  } catch {
  }
}
async function notifyAuthClose(reason, options) {
  var _a;
  try {
    await ((_a = options.onClose) == null ? void 0 : _a.call(options, reason));
  } catch {
  }
}
...
function renderEmailLoginCard(windowRef, uiOptions) {
  ...
  const passwordToggle = (_a = password.suffixNode) == null ? void 0 : _a.querySelector("button");
  passwordToggle == null ? void 0 : passwordToggle.addEventListener("click", () => {
...
function createInputRow(documentRef, options) {
  ...
  if (options.suffixLabel) {
    ...
    suffixButton.addEventListener("click", () => {
      var _a;
      void ((_a = options.onSuffixClick) == null ? void 0 : _a.call(options));
    });
...
function showToast(documentRef, type, message) {
  ...
  const timer = window.setTimeout(dismiss, 3e3);
...
function formatAuthErrorMessage(error, context, options) {
  var _a, _b, _c;
  try {
    const customMessage = (_a = options.formatErrorMessage) == null ? void 0 : _a.call(options, error, context).trim();
    if (customMessage)
      return customMessage;
  } catch {
    return ((_b = options.messages) == null ? void 0 : _b.genericErrorMessage) ?? "Something went wrong. Please try again.";
  }
  if (error instanceof Error && error.message.trim())
    return error.message;
  const message = String(error).trim();
  return message || ((_c = options.messages) == null ? void 0 : _c.genericErrorMessage) || "Something went wrong. Please try again.";
}
async function notifyAuthError(error, context, options) {
  var _a;
  try {
    await ((_a = options.onError) == null ? void 0 : _a.call(options, error, context));
  } catch {
  }
}
...
function metaContent(documentRef, attr, value) {
  ...
  return ((_a = element == null ? void 0 : element.getAttribute("content")) == null ? void 0 : _a.trim()) ?? "";
}
function linkHref(documentRef, selector) {
  var _a;
  const href = ((_a = documentRef.querySelector(selector)) == null ? void 0 : _a.href) ?? "";
  return href.trim();
}
...
function generateState(windowRef) {
  var _a, _b;
  if ((_a = windowRef.crypto) == null ? void 0 : _a.randomUUID)
    return windowRef.crypto.randomUUID();
  if (!((_b = windowRef.crypto) == null ? void 0 : _b.getRandomValues)) {
...
//# sourceMappingURL=@verdent_auth-js.js.map
